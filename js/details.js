...
$(document).ready(function () {
    ...
    // set bodymovin
    let scrollAnim1 = bodymovin.loadAnimation(openingAnimData1);
    // Set Pin Page 設定一個畫面的鎖定pin，開始點是top top，意思是當物件的top碰觸到畫面的top就開始播
    ScrollTrigger.create({
        trigger: "#page3",
        start: "top top",
    ...
    scrollAnim1.addEventListener("DOMLoaded", onDOMLoaded1);

    // // Frame-by-frame play
    function onDOMLoaded1() {
        timeLine_page3.to(
            {
                frame: 0,
            },
            3,
            {
                frame: scrollAnim1.totalFrames - 1,
                onUpdate: function () {
                    scrollAnim1.goToAndStop(Math.round(this.targets()[0].frame), true);
                    $("#counter").html(Math.round(this.targets()[0].frame));
                },
                ease: Linear.easeNone,
            }
...
//不管她的變數名稱是甚麼，反正他就是要播下面這個動畫，只是觸發的東西變了，變得不是物件本身
    ...
    // };
    // let scrollAnim1 = bodymovin.loadAnimation(openingAnimData1);
    // scrollAnim1.addEventListener("DOMLoaded", onDOMLoaded);
